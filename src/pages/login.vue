<script setup lang="ts">
  ...
  const {
    serverError, handleServerError, realtimeErrors, handleLoginForm
  } = useFormErrors()

  const router = useRouter()

  // the deep, option help us track al changes inside nested formData obj
      ...
      <CardContent>
        ...
        <form class="grid gap-4" @submit.prevent="signin">
          <div class="grid gap-2">
            ...
            <Input v-model="formData.email" type="email" placeholder="johndoe19@example.com"
              required :class="{'border-red-500': serverError}"
            />
